...
function getAlltheCodez(){
	...
	if($('#updateJava').is(':checked')) jav = '\n<script>\n'+myCodeJava.getValue()+'\n<\/script>';
	return value = '<html>\n<head>\n'+head+'\n</head>\n<body>\n'+body+'\n<style>\n'+css+'\n</style>'+jav+'\n</body>\n</html>';
}
function updatePreview0(){
	anterior = $('#updateJava').is(':checked');
	$('#updateJava').prop({checked:true});
	updatePreview();
	$('#updateJava').prop({checked:false});
	if(anterior) $('#updateJava').prop({checked:true});
}
