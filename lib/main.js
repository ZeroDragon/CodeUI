		...
		for (var i = 0; i < archivos.length; i++) {
			...
			$('#innerlegaleria').append(
				$('<div/>',{
					...
					$('<div/>',{class:"hname"}).append(archivos[i][0])
				)
			);
		};
	})
...
function elim( file ){
	...
	function(data){
		if(data!='Ok') muestraerror(data);
		getImagenes();
	});
}
function muestraerror(elerror){
	$('#errores').removeClass('errores');
	$('#errores').addClass('errores');
	$('#errores').text(elerror);
	$('#errores').fadeIn('slow');
...
$("#formsubmit").click(function () {
    ...
    $("#postiframe").load(function () {
    	data = $("#postiframe")[0].contentDocument.body.innerHTML;
    	if(data!='Ok') muestraerror(data);
    	getImagenes();
